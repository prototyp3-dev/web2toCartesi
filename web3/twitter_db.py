...
user_functions = set(("create_user", "check_user"))
...
def read_function(f_name):
    return f_name in read_functions

def user_function(f_name):
    return f_name in user_functions
...
#                                                                 #
###################################################################
def create_user(conn, username, address, join_date, emoji):
    address = sha256(address.encode()).hexdigest()

    sql = ''' INSERT INTO users(username, address, join_date, emoji)
            VALUES(?, ?, ?, ?) '''
    cur = conn.cursor()

    cur.execute(sql, (username, address, join_date, emoji))
    conn.commit()
...
#                                                                 #
###################################################################
def check_user(conn, username, address):
    sql = ''' SELECT username, join_date, emoji FROM users WHERE username = ? AND address = ? '''
    cur = conn.cursor()

    address = sha256(address.encode()).hexdigest()
    
    cur.execute(sql, (username, address))
