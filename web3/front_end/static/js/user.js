...
function sign_up(username, user_emoji) {
    ...
    let input = JSON.stringify(db_operation)
    input_contract_add_input(input, (res) => {
        if (res[0].success) {
            // create "user" cookie
            document.cookie = `user=${username},${join_date},${user_emoji};sameSite=Strict`
        ...
        user_action_form_loading[btn_id] = {"success": false, "error": error}
    })
}

function log_in(username, address) {
    let db_operation = {
        "f_calls": {
            "check_user": {"username": username,"address": address}
        }
    }
    inspect_db(db_operation, (res) => {
        if (res[0].success) {
            if (res[0].result && res[0].result.length) {
                // create "user" cookie
                document.cookie = `user=${res[0].result};sameSite=Strict`
